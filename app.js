...
const slides = [
  ...
  },
  {
    ...
    afterChain: ["接入校招数据库", "AI 将清洗规则写成自动化脚本", "用户输入专业实时匹配数据", "岗位大类、城市热力、能力雷达交互分析", "大模型按筛选条件和业务提示词定向解读", "AI 实时解读"],
  ...
  },
  {
    ...
    layout: "trust",
    mechanisms: [
      ["专业字典", "以官方标准专业名称为统一基准，无论企业招聘的专业表述是否规范、是否穷举完整，都能实现岗位专业要求的全覆盖、高精准、高效率匹配，有效减少岗位错配与遗漏。"],
      ["岗位大类字典", "把工作内容、核心能力一致但岗位名称各异的职位进行统一归类，可用于岗位聚类分析与相似职位智能推荐。"],
      ["岗位能力模型", "结合真实职责、任职要求、关键词识别、语义判断和服务经验。"],
      ["AI 解读", "接入 DeepSeek 模型，结合业务视角提示词进行实时数据的解读。"]
    ],
    equation: "专业字典 + 岗位大类字典 + 真实招聘数据 + DeepSeek 实时解读 = 稳定、可解释的就业分析结果",
  ...
  },
  {
    ...
    values: [
      ...
      ["对互联派", "将多年校招数据、导师经验和后端分析逻辑转化为可复用系统能力。"]
    ],
